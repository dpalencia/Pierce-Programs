...
void payoff_calc(double & sum, double S_adjust, double K, double v, double T) {
	double gauss_bm = gaussian_box_muller();
	double S_cur = S_adjust * exp(sqrt(v * v * T) * gauss_bm);
	sum = sum + max(S_cur - K, 0.0);
}
...
double monte_carlo_call_price(const int& num_sims, const double& S, const double& K, const double& r, const double& v, const double& T) {
	...
	vector<thread> vt;
	void (*multi_payoff_calc)(double &, double, double, double, double) = payoff_calc;
	for (unsigned i = 0; i < num_sims / thread().hardware_concurrency(); i++) {
		vt.push_back(thread(multi_payoff_calc, payoff_sum, S_adjust, K, v, T));
	}
	for (unsigned i = 0; i < num_sims / thread().hardware_concurrency(); i++)
		vt[i].join();
